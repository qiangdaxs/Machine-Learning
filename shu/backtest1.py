...
start = '2013-2-11 14:30:00'
end = '2014-2-10 21:00:00'
#===========sub list of 10 stocks in SP500==============================================
stocks = ['SBUX','NFLX','EBAY','CSCO','CA','BIIB','BBBY','ADP']

nsdq=['SYMC', 'WDC','NFLX', 'CERN', 'WFM','ALXN', 'GRMN', 'SRCL', 'BIIB', 'REGN', 'AMAT', 'COST', 'FISV', 'NVDA', 'ADBE', 
...
signal_data = signal_data[signal_data['entities_ticker_1'].isin(nsdq)]
...
def handle_data(context, data):
    ...
    long_bulls_short_bears(context,data)
    context.pnl[get_datetime().strftime('%Y-%m-%d %H:%M')]=context.portfolio.pnl
    context.date.append(get_datetime().strftime('%Y-%m-%d %H:%M'))
...
def long_bulls_short_bears(context,data):
    ...
    for stock in context.stocks:
        ticker=(minute_symbol['entities_ticker_1']==stock)
        content=minute_symbol[ticker]
        #=====article_sentiment is the average article sentiment at that time========
        article_sentiment=np.mean(content['article_sentiment'])
         
        #===== event_source_rank is the average  at that time========================
        event_source_rank=np.mean(content['event_source_rank_1'])

        #===== event_impact_score is the average  at that time=======================
        event_impact_score=np.mean(content['event_impact_score_entity_1'])
        
        if np.isnan(article_sentiment) == False and np.isnan(event_source_rank) == False and np.isnan(event_impact_score) == False:
            
            if article_sentiment > context.upper_bound and event_source_rank > context.rank_bound and event_impact_score > context.impact_bound:
                context.bulls.append(stock)

            if article_sentiment < context.lower_bound and event_source_rank > context.rank_bound and event_impact_score > context.impact_bound: 
                context.bears.append(stock)
    
    #==================Do the transaction 30 min before the market close=============
    if get_datetime().strftime('%H:%M') == '15:00':
        ...
        if context.portfolio.positions == {}:
            ...
            sell_time = get_trading_days(start,end)[index+2]
    ...
    if get_datetime().strftime('%Y-%m-%d') == context.sell_time and get_datetime().strftime('%H:%M') == '19:45':
        #import pdb;pdb.set_trace()
        context.sell_time=''
        for stock in context.stocks:
