...
__created__ = '2015-05-15 10:09:46 -0400'
...
def change_dates(fetcher_data):
    #: Not quite sure if this + timedelta(days=1) should go first or second?
    fetcher_data['start_date'] = pd.to_datetime(fetcher_data['start_date'], pytz.timezone('US/Eastern')) + timedelta(days=1)
    fetcher_data['start_date'] = fetcher_data['start_date'].apply(lambda date: date - timedelta(days=1) if (date.hour < 9 or (date.hour == 9 and date.minute < 30)) else date)
