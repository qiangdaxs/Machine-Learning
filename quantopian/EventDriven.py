...
def initialize(context):
    ...
    schedule_function(calculate_bulls_and_bears,
...
def calculate_bulls_and_bears(context, data):
    ...
    for stock in context.stocks:
        if ('article_sentiment' and 'event_impact_score_entity_1' and 'overall_source_rank') in data[stock]:
            ...
            if data[stock]['enter_date'].month == today.month and data[stock]['enter_date'].year == today.year and data[stock]['enter_date'].day == today.day:
                if (data[stock]['article_sentiment'] > context.upper_bound) and (data[stock]['event_impact_score_entity_1'] > context.upper_bound_a) and (data[stock]['overall_source_rank'] > context.upper_bound_c) and (data[stock]['event_groups_type_1'] == 'Financial Ratings'):
                    bulls.append(stock)
                 
                
                elif (data[stock]['article_sentiment'] < context.lower_bound) and (data[stock]['event_impact_score_entity_1'] > context.lower_bound_a) and (data[stock]['overall_source_rank'] > context.lower_bound_c) and (data[stock]['event_groups_type_1'] == 'Financial Ratings'):
                    bears.append(stock)
